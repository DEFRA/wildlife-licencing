  ...
  it('runs initialisation', done => {
    jest.isolateModules(() => {
      try {
        ...
        jest.mock('../applications-database-writer.js')

        const { SEQUELIZE } = require('@defra/wls-connectors-lib')
        const { createModels } = require('@defra/wls-database-model')
        const { applicationReadStream } = require('@defra/wls-powerapps-lib')
        const { applicationsDatabaseWriter } = require('../applications-database-writer.js')
        ...
        applicationsDatabaseWriter.mockImplementation(() => Promise.resolve())
        ...
        setImmediate(() => {
          expect(applicationsDatabaseWriter).toHaveBeenCalled()
  ...
  it('exists on error', done => {
    jest.isolateModules(() => {
      try {
        ...
        createModels.mockImplementation(() => Promise.resolve())
        applicationReadStream.mockImplementation(() => ({}))
        applicationsDatabaseWriter.mockImplementation(() => Promise.reject(new Error()))
        const mockExit = jest.fn()
        jest.spyOn(process, 'exit')
          .mockImplementation(code => mockExit(code))
