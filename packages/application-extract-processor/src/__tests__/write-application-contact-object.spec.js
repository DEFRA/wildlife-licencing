...
const keys = [
  ...
  },
  {
    apiTable: 'contacts',
    apiKey: null,
    apiBasePath: 'application.additionalApplicant',
    powerAppsTable: 'contacts',
    contentId: null,
    powerAppsKey: '6dd96d9f-db01-ed11-82e5-002248c5c45b'
  },
  {
    apiTable: 'contacts',
    apiKey: null,
    apiBasePath: 'application.additionalEcologist',
    powerAppsTable: 'contacts',
    contentId: null,
    powerAppsKey: '7terfg45r-db01-ed11-82e5-002248c5c45b'
  },
  {
    apiTable: 'contacts',
    apiKey: null,
...
describe('The application-contact extract processor: write-application-contact-object', () => {
  ...
  it('creates an application additionalApplicant', async () => {
    const mockCreate = jest.fn()
    jest.doMock('@defra/wls-database-model', () => ({
      models: {
        applications: {
        ...
        contacts: {
          findOne: jest.fn()
            .mockReturnValueOnce({ id: '5718ad54-bab7-4a78-8ca9-dcf722117a45' })
            .mockReturnValue(null)
        },
        applicationContacts: {
          findOne: jest.fn(() => null),
  ...
  it('creates an application additionalEcologist', async () => {
    const mockCreate = jest.fn()
    jest.doMock('@defra/wls-database-model', () => ({
      models: {
        applications: {
  ...
  it('creates an payer', async () => {
    ...
    const { writeApplicationContactObject } = await import('../write-application-contact-object.js')
    const result = await writeApplicationContactObject({ data: { }, keys: [keys[0], keys[1], keys[5]] })
    expect(result).toEqual({ error: 0, insert: 2, pending: 0, update: 0 })
    expect(mockCreate).toHaveBeenCalledWith({
      id: expect.any(String),
      applicationId: '5eac8c64-7fa6-4418-bf24-ea2766ce802a',
