...
import {
  licenceTypeMap, A24, getStatus, tasks, updateStatusCache,
  STATUS_VALUES, decorateMap, getProgress
} from './licence-type-map.js'

export const getData = async request => {
  // Placeholder - this will be replaced by a state-machine
  const eligibilityStatus = await getStatus(tasks.ELIGIBILITY_CHECK)(request)
  if (eligibilityStatus === STATUS_VALUES.CANNOT_START_YET) {
    await updateStatusCache(request, tasks.ELIGIBILITY_CHECK, STATUS_VALUES.NOT_STARTED)
  }
