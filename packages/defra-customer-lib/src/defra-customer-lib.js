...
const USER_QUERY = userId => 'contacts(' + userId + ')?$select=emailaddress1,fullname,telephone1,defra_uniquereference' +
  ',defra_addrcorsubbuildingname,defra_addrcorbuildingname,defra_addrcorbuildingnumber,defra_addrcorstreet,' +
  'defra_addrcorlocality,defra_addrcortown,defra_addrcorcounty,defra_addrcorpostcode,defra_addrcoruprn'
const ORGANISATION_QUERY = organisationId => 'accounts(' + organisationId + ')?$select=name,emailaddress1,defra_addrregsubbuildingname,' +
  'defra_addrregbuildingname,defra_addrregbuildingnumber,defra_addrregstreet,defra_addrreglocality,defra_addrregtown' +
  ',defra_addrregcounty,defra_addrregpostcode,defra_addrreguprn'
 ...
 * @returns {{address:
 * {
 * country: string, uprn: string, town: string, locality: string, county: string, postcode: string,
 * dependentLocality: string, subBuildingName: string, buildingName: string, street: string,
 * addressLine1: string, buildingNumber: string, addressLine2: string
 * },
 * fullName: *,
 * contactDetails: {phone: (string|null|*), email: *}}}
 */
