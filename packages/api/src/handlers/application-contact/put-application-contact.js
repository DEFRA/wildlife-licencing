...
export default async (context, req, h) => {
  try {
    ...
    const [applicationContact, created] =
      await models.applicationContacts.findOrCreate({
    ...
    if (created) {
      ...
      const [, updatedApplicationContact] =
        await models.applicationContacts.update(
          {
            applicationId,
            contactId,
            contactRole,
      ...
      const responseBody = prepareResponse(
        updatedApplicationContact[0].dataValues
      )
      await cache.save(req.path, responseBody)
    ...
    console.error(
      'Error inserting into, or updating, the APPLICATION-CONTACTS table',
      err
    )
