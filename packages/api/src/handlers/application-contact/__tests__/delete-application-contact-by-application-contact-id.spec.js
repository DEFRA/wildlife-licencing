...
let models
let deleteApplicationContactByApplicationContactId
let cache

describe('The deleteApplicationContactByApplicationContactId handler', () => {
  beforeAll(async () => {
  ...
  it('returns a 404 on id not found', async () => {
    cache.delete = jest.fn()
    models.applicationContacts = { destroy: jest.fn(() => 0) }
    await deleteApplicationContactByApplicationContactId(context, req, h)
