...
export default async (context, req, h) => {
  try {
    ...
    if (!application) {
      return h
        .response({
          code: 404,
          error: {
            description: `post, applicationId: ${applicationId} not found`
          }
        })
    ...
    if (!designatedSite) {
      return h
        .response({
          code: 400,
          error: {
            description: `post, designatedSiteId: ${designatedSiteId} not found`
          }
        })
        .type(APPLICATION_JSON)
        .code(400)
    ...
    await cache.save(
      `/application/${applicationId}/designated-site/${dataValues.id}`,
      responseBody
    )
