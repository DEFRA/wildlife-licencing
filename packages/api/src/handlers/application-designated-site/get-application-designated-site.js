...
export default async (context, req, h) => {
  try {
    ...
    const applicationDesignatedSite =
      await models.applicationDesignatedSites.findByPk(
        applicationDesignatedSiteId
      )
    if (!applicationDesignatedSite) {
      return h
        .response({
          code: 404,
          error: {
            description: `applicationDesignatedSiteId: ${applicationDesignatedSiteId} not found`
          }
        })
        .code(404)
    }
