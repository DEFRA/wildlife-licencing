...
import {
  prepareResponse,
  alwaysExclude
} from './application-designated-site-proc.js'
...
export default async (context, req, h) => {
  try {
    ...
    const designatedSite =
      await models.designatedSites.findByPk(designatedSiteId)
    ...
    if (!application) {
      return h
        .response({
          code: 404,
          error: {
            description: `put, applicationId: ${applicationId} not found`
          }
        })
    ...
    if (!designatedSite) {
      return h
        .response({
          code: 400,
          error: {
            description: `put, designatedSiteId: ${designatedSiteId} not found`
          }
        })
        .type(APPLICATION_JSON)
        .code(400)
    ...
    const [applicationDesignatedSite, created] =
      await models.applicationDesignatedSites.findOrCreate({
    ...
    if (created) {
      ...
      const [, updatedApplicationDesignatedSite] =
      ...
      const responseBody = prepareResponse(
        updatedApplicationDesignatedSite[0].dataValues
      )
      await cache.save(req.path, responseBody)
