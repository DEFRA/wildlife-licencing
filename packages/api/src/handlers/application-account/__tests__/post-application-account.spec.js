...
describe('post-application-account-handler', () => {
  beforeEach(() => {
    jest.resetModules()
    jest.clearAllMocks()
  })
      ...
      payload:
        {
          ...
          accountId: 'bbe0a988-2efe-41d5-901d-cad600fef2fd',
          role: 'ECOLOGIST'
        }
    }
    const context = {}
    const postSiteUser = (await import('../post-application-account.js')).default
  ...
  it('returns a status 409 if the application-account already exists', async () => {
    jest.doMock('@defra/wls-database-model', () => ({
      models: {
        applications: {
          findByPk: jest.fn(() => ({ id: '1ee0737e-f97d-4f79-8225-81b6014ce37e' }))
