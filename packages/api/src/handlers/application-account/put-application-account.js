...
export default async (context, req, h) => {
  try {
    ...
    if (!account) {
      return h.response({ code: 400, error: { description: `accountId: ${accountId} not found` } })
        .type(APPLICATION_JSON)
        .code(400)
    }

    const [applicationAccount, created] = await models.applicationAccounts.findOrCreate({
    ...
    if (created) {
      ...
      const [, updatedApplicationAccount] = await models.applicationAccounts.update({
        applicationId,
        accountId,
        accountRole,
        updateStatus: 'L'
      ...
      const responseBody = prepareResponse(updatedApplicationAccount[0].dataValues)
      await cache.save(req.path, responseBody)
      return h.response(responseBody)
        .type(APPLICATION_JSON)
        .code(200)
    ...
    console.error('Error inserting into, or updating, the APPLICATION-ACCOUNTS table', err)
