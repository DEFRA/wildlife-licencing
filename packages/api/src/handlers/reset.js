  ...
  try {
    ...
    if (!user) {
      return h
        .response({
          code: 400,
          error: { description: `username: ${username} not found` }
        })
    ...
    response.applicationContacts = await models.applicationContacts.destroy(
      inApplicationIds(applicationIds)
    )
    response.applicationAccounts = await models.applicationAccounts.destroy(
      inApplicationIds(applicationIds)
    )
    response.applicationSites = await models.applicationSites.destroy(
      inApplicationIds(applicationIds)
    )
    response.permissions = await models.permissions.destroy(
      inApplicationIds(applicationIds)
    )
    ...
    response.habitatSites = await models.habitatSites.destroy(
      inApplicationIds(applicationIds)
    )
    response.returns = await models.returns.destroy({
      where: { licenceId: { [Op.in]: licenceIds } }
    })
    response.licences = await models.licences.destroy(inIds(licenceIds))
    response.previousLicences = await models.previousLicences.destroy(
      inApplicationIds(applicationIds)
    )
    response.applicationUploads = await models.applicationUploads.destroy(
      inApplicationIds(applicationIds)
    )
    response.applicationDesignatedSites =
      await models.applicationDesignatedSites.destroy(
        inApplicationIds(applicationIds)
      )
    response.applications = await models.applications.destroy(
      inIds(applicationIds)
    )
